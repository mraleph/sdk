...
bool AsyncAwareStackUnwinder::Unwind(
  ...
  for (; !awaiter_frame_.closure.IsNull(); UnwindToAwaiter()) {
    ...
    if (awaiter_frame_.next.IsSuspendState()) {
      ...
      // will handle successful completion. The closure is not executing yet
      // so we are going to use `1` as a pc_offset.
      code_ = function_.EnsureHasCode();
      RELEASE_ASSERT(!code_.IsNull());
      const uword pc_offset =
          (function_.entry_point() + 1) - code_.PayloadStart();
      handle_frame({nullptr, code_, pc_offset, awaiter_frame_.closure,
                    awaiter_frame_.has_catch_error});
    }
  }
...
void AsyncAwareStackUnwinder::UnwindAwaiterFrame() {
  ...
  while (!awaiter_frame_.closure.IsNull()) {
    ...
    const auto [awaiter_link_depth, awaiter_link_index] =
        function_.awaiter_link();
    if (awaiter_link_depth != ClosureData::kNoAwaiterLinkDepth) {
      intptr_t depth = awaiter_link_depth;
      while (depth-- > 0) {
        context_ = context_.parent();
      }

      const Object& object = Object::Handle(context_.At(awaiter_link_index));
      if (object.IsClosure()) {
...
bool StackTraceUtils::GetSuspendState(const Closure& closure,
  ...
  if (function.awaiter_link().first != ClosureData::kNoAwaiterLinkDepth) {
    const auto [awaiter_link_depth, awaiter_link_index] =
    ...
    const Object& link = Object::Handle(context.At(awaiter_link_index));
