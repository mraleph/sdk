...
LocationSummary* Instruction::MakeCallSummary(Zone* zone,
  ...
  LocationSummary* result =
      ((locs == nullptr)
           ? (new (zone) LocationSummary(zone, instr->InputCount(), 0,
                                         LocationSummary::kCall))
           : locs);
  // Allocate the outgoing arguments under assumption that they all go to the
  // stack.
  ASSERT(instr->ArgumentCount() <= instr->InputCount());
  ...
  for (intptr_t i = arg_count - 1; i >= 0; --i) {
    ...
    result->set_in(i, rep == kUnboxedDouble
                          ? Location::DoubleStackSlot(sp_relative_index, SPREG)
                          : Location::StackSlot(sp_relative_index, SPREG));
...
LocationSummary* ClosureCallInstr::MakeLocationSummary(Zone* zone,
  ...
  summary->set_in(
      InputCount() - 1,
      Location::RegisterLocation(FLAG_precompiled_mode ? R0 : FUNCTION_REG));
  return summary;
}
