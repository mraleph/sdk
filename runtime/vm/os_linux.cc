...
class JitDumpCodeObserver : public CodeObserver {
 public:
  ...
  ~JitDumpCodeObserver() {
    ...
    SafeFileClose(&out_file_);
  }

  static void SafeFileClose(FILE** f) {
    if (*f != nullptr) {
      fclose(*f);
      *f = nullptr;
    }
  ...
  virtual void Notify(const char* name,
    ...
    if ((comments != nullptr) && (comments->Length() > 0)) {
      ...
      char* comments_file_name = OS::SCreate(nullptr, "/tmp/jit-%" Pd "-%" Pd ".cmts", pid_, code_id_);
      ...
      info.size = sizeof(info) + entry_count * (sizeof(DebugInfoEntry) + filename_length + 1);
      ...
      for (intptr_t i = 0, len = comments->Length(); i < len; ) {
        const intptr_t pc_offset = comments->PCOffsetAt(i);
        while (i < len && comments->PCOffsetAt(i) == pc_offset) {
          WriteLn(comments_file, comments->CommentAt(i));
          line_number++;
          i++;
        }
 ...
 private:
  ...
  void WriteLn(FILE* f, const char* comment) {
    if (strstr(comment, "\n") != NULL) {
      UNREACHABLE();
    }
    fputs(comment, f);
  ...
  // char* comments_file_name_ = nullptr;
  // FILE* comments_file_ = nullptr;
  // intptr_t line_number_ = 0;
